...
PRIMARY = "#5B8A9A"     # muted teal-blue
ACCENT = "#E8A87C"      # warm sand
SOFT_BG = "#F7F5F2"     # warm off-white
SUCCESS = "#7FB08A"     # sage green
CARD_BORDER = "#E4DFD8"
...
DEFAULT_SETTINGS = {
    ...
    "water_goal_ml": "2000",
    "protein_goal_g": "100",
    "streak_grace": "1",            # missed days allowed before streak resets
}
...
STREAK_GRACE = int(float(SETTINGS.get("streak_grace", 1)))
...
def get_weekly_average_steps(target_date_str):
    ...
    start = (target - datetime.timedelta(days=7)).strftime("%Y-%m-%d")
    df = pd.read_sql_query("SELECT steps FROM daily_logs WHERE date >= ? AND date < ?", conn, params=(start, target_date_str))
    conn.close()
...
def calculate_streak(end_date_str, grace_days=1):
    """Counts consecutive days (allowing `grace_days` gaps) with any log activity.
    Streaks celebrate check-ins; they never explicitly call out 'broken' streaks."""
    conn = get_db_connection()
    df_dates = pd.read_sql_query("""
        SELECT date FROM daily_logs WHERE water_ml > 0 OR steps > 0 OR mental_state IS NOT NULL OR is_rest_day = 1
        UNION SELECT date FROM activities
        UNION SELECT date FROM calisthenics
        UNION SELECT date FROM therapies
    """, conn)
    conn.close()
    if df_dates.empty:
        return 0
    logged_dates = set(pd.to_datetime(df_dates['date']).dt.date)
    ...
    while True:
        if cursor in logged_dates:
            ...
            if gap_used < grace_days and cursor != end:
                gap_used += 1
                cursor -= datetime.timedelta(days=1)
                continue
            break
...
REST_FACTS = [
    ...
    "Consistency in chronic illness looks like showing up in small, sustainable ways — not unbroken streaks of intensity.",
    "Pacing (the 'spoon theory' approach) is a recognized management strategy for EDS and dysautonomia, not a consolation prize.",
    "Every log entry — even 'I rested' — is useful data for you and your care team. It all counts.",
]
...
col_d1, col_d2, col_d3 = st.columns([3, 1, 1.4])
with col_d1:
...
with col_d3:
    st.write("")
    streak = calculate_streak(datetime.date.today().strftime("%Y-%m-%d"), STREAK_GRACE)
    if streak > 0:
        st.markdown(f'<span class="streak-badge">🔥 {streak}-day check-in streak</span>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    c1.metric("7-day average", int(avg_steps))
        ...
        if st.button("Save Steps"):
            ...
            if avg_steps > 0:
                if steps > caution_steps:
                    st.warning("💛 A higher-than-usual day. Nothing wrong with that — just a gentle nudge to prioritize rest tomorrow, hydrate, and consider electrolytes.")
    ...
    if st.button("Log Calisthenics"):
        ...
        if check_calisthenics_comfort(cal_type, date_str):
            st.balloons()
            st.success(f"🌟 Your last 3 sessions for {cal_type} averaged {COMFORT_THRESHOLD}+ comfort. You're physically cleared for the next progression — only if you feel mentally ready too. No rush.")
...
with tabs[2]:
    ...
    if df_range.empty or df_range['steps'].fillna(0).sum() == 0 and df_range['water_ml'].fillna(0).sum() == 0:
        ...
        activity_days = set(df_daily.loc[(df_daily['water_ml'] > 0) | (df_daily['steps'] > 0) | (df_daily['is_rest_day'] == 1), 'date'])
        rest_days = set(df_daily.loc[df_daily['is_rest_day'] == 1, 'date'])
        strip = df_range[['date']].copy()
        strip['date_str'] = strip['date'].dt.strftime("%Y-%m-%d")
        strip['status'] = strip['date_str'].apply(lambda d: "Rest" if d in rest_days else ("Logged" if d in activity_days else "No entry"))
        fig_strip = px.scatter(strip, x='date', y=[1]*len(strip), color='status',
                                color_discrete_map={"Logged": PRIMARY, "Rest": ACCENT, "No entry": "#DDDDDD"},
        ...
        st.caption("Gaps aren't failures — they're information. What matters is that the overall pattern keeps you engaged with your body's needs.")
    ...
    total_checkins = len(df_daily[(df_daily['water_ml'] > 0) | (df_daily['steps'] > 0) | (df_daily['is_rest_day'] == 1)])
    ...
    e3.metric("Recovery sessions", total_therapy_sessions)
    e4.metric("Movement logs", total_activities_logged)
    st.caption("Every number above represents a choice to show up for yourself. None of them need to be higher to count.")
...
with tabs[3]:
    ...
    with st.form("settings_form"):
        ...
        new_grace = st.number_input("Missed days allowed before streak resets", min_value=0, max_value=7, value=STREAK_GRACE, step=1,
                                     help="A small grace window keeps streaks compassionate rather than punishing.")
        ...
        if st.form_submit_button("💾 Save Settings"):
            ...
            set_setting("streak_grace", new_grace)
